...
export function PresetSelector(props: PopoverProps) {
  ...
  handlePresetChange();
}, [pgState.selectedPresetId, lastPresetId, shouldConnect, disconnect, connect]);
        ...
        <PopoverContent className="w-[300px] p-0">
          <Command>
            ...
            <CommandList className="max-h-[320px]">
              {pgState.presets.length > 0 && (
                <CommandGroup heading="Saved">
                  {pgState.presets.map((preset: Preset) => (
                    <CommandItem
                      key={preset.id}
                      value={preset.id}
              ...
              {Object.values(PresetGroup).map((group) => (
                <CommandGroup key={group} heading={group}>
                  ...
                  {helpers.getDefaultPresets(pgState)
 .filter((preset) => preset.defaultGroup)
  .map((preset) => (
                      <CommandItem
                        key={preset.id}
                        value={preset.id}
